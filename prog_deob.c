 ...
 * first 956 bytes are text, probably font data and possibly holes
...
int main() {
	...
	for (G="SJYXHFUJ735";*G;)
		C(*G++-5);C(3);V(1);
	for (V(j=z);j<21*3;j++) {
		k=257;V(63777);V(k<<2);V(M(j,32)?11:511);V(z);C(22*2);V(i=f=z);V(z);V(W);V(H);V(1<<11);r=
	G=I+W*H;L(t=T;i<1<<21;i++)T[i]=i<256?i:-1;E(256);L(i=-1;++i<W*H;t=T+Z*256+256)c=I[i]?I[i]*31-31:(31<
		j?j-31:31-j),Z=c[t[c]<z?E(Z),k<(1<<12)-2?t[c]=++k,T:T:t];E(Z);E(257);L(G++;k=G-r>X?X:G-r
				,C(k),k;)L(;k--;C(*r++/*---#$%&04689@ABDOPQRabdegopq---*/));
	}
	C(59);
	return(z);
}
